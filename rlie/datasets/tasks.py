    ...
    def __init__(
        ...
        if data_dir is None:
            ...
            self.data_dir = os.path.normpath(
                os.path.join(os.path.dirname(config_path), data_dir)
            )
        ...
        if prompts_path is None:
            prompts_data = data.get("prompt_templates")
            if prompts_data is None:
                prompts_file = os.path.join(os.path.dirname(config_path), "prompts.yaml")
                if not os.path.exists(prompts_file):
                    raise FileNotFoundError(
                        f"Could not locate prompt templates for task config {config_path}"
                    )
                with open(prompts_file, "r") as f:
                    prompts_data = yaml.safe_load(f)
        else:
            prompts_file = os.path.normpath(
                os.path.join(os.path.dirname(config_path), prompts_path)
            )
            with open(prompts_file, "r") as f:
                prompts_data = yaml.safe_load(f)

        if not isinstance(prompts_data, dict):
            raise ValueError(f"Prompt templates must be a mapping in task config {config_path}")
        self.prompt_template = prompts_data.get("prompt_templates", prompts_data)
